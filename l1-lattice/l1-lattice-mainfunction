...
function est_spa(data)
    ...
    A = [(i == j ? sum(W[:,i]) : 0) for i=1:M,j=1:M]
    W_a = copy(W .- diagm(ones(M).*0.1))
    W_A = A .- W_a
    ...
    phicof = ones(M) .* 0.1#rand(MvNormal(phitau * R_phi),1)
    ...
    while iter < niter + 1
        ...
        #eps_phicof = 0.5
        betaxcov =  xcov_non * theta_non
        phi_true = phicof_sampler(phicof, eps_phicof, W_A, z, spaind, betaxcov, te2, phitau)
        phicof = phi_true["phicof"]
        #accept_phi[iter] = phi_true["accept"]

        #spatial parameter--phitau
        phitau = rand(Gamma(0.5 * (M - g) + a_phitau,1/(0.5 * (phicof' * W_A * phicof)[1] + b_phitau)),1)[1]
        ...
        if (iter > 1000) + (iter % 500 == 0) == 2
            ...
            eps_w = epsilon_change(iter, eps_w, accept_theta)
            c_phi = epsilon_change(iter, c_phi, accept_theta)
            #println(c_beta)
            #println(c_gamma)
            #println(c_cure)
            #println(c_sur)
