...
function Data_generation(seed, n, R_phi, true_beta, rho, fun1, exact_rate, tau)
    Random.seed!(seed)
    N = size(R_phi)[1]
    p = length(true_beta)
    ...
    area = vcat(repeat(collect(1:N),2),rand(DiscreteUniform(1,N),n-2*N))
    phicof = sqrt(phitau) .* (sqrt(R_phi) * randn(N))
    spaind = zeros(n,N) 
    for i in 1:N
...
function log_Posterior_phi(spatialcof_true; betaxcov, z, te2, spaind, R_phi, phitau)
    sum1 = z' * spaind * spatialcof_true
    sum2 = te2' * exp.(betaxcov .+ spaind * spatialcof_true)
    sum3 = (spatialcof_true' * R_phi * spatialcof_true) ./ (2 .*phitau)
    result = sum1.-sum2.-sum3
    return result
end
function partial_phi(spatialcof_true; betaxcov, z, te2, spaind, R_phi, phitau)
    sum1 = spaind' * z
    sum2 = spaind' *  (te2 .* exp.(betaxcov + spaind * spatialcof_true))
    sum3 = R_phi * spatialcof_true ./ phitau
    result = sum1-sum2-sum3
    return result
end
function Hessian_phi(spatialcof_true; betaxcov, z,te2, spaind, R_phi, phitau) 
    hessian1 = spaind' * (spaind .* (te2 .* exp.(betaxcov + spaind * spatialcof_true)))
    hessian2 = R_phi ./ phitau
    result = hessian1 + hessian2
    return(result)
end
function epsilon_change(iter, c_real, accept)
    if sum(accept)/iter > 0.5
        c_real = c_real * 1.2
    elseif sum(accept)/iter < 0.3
        c_real = c_real / 1.5
    end
    return c_real
end
